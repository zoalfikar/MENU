<template>
    ...
    <div v-else>
        ...
        <div class="items">
            <div class="item add" @click="()=>showForm=true">add new item here +</div>
            <div class="item" v-for="it  in currentItems" v-bind:key="it.id">
                <p>{{it.title}}</p>
                <p>price &nbsp; {{it.price}}</p>
    ...
    <div :style="showForm?'':'display:none'" :class="'category-form'">
            ...
            <input v-model="newItem.price" type="number" placeholder="format 0.00">

            <label for="discountPercent">discount</label>
            <input v-model="newItem.discountPercent" type="number" >
...
export default {
    ...
    watch:{
        currentParentCategory:function (newV, oldV) {
            if (newV)
            this.newItem.categoryId  = newV.id;
            else
    ...
    methods:{
        ...
        save:function(){
                store.dispatch('createItem' ,this.newItem)
            },
         historyPoint(){
